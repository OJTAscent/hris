...
@section('content')
    <div class="container">
        <div class="card shadow">
            <div class="card-body">
                <form action="{{ route('roles.store') }}" method="POST" class="row g-3 needs-validation p-4" novalidate>
                    @csrf
                    <h5 class="card-title">
                        {{ $title }}
                        ...
                        @if ($mode == 'show')
                            <a href="{{ route('roles.edit', $role->id) }}" class="btn btn-primary me-2 float-end"> Edit</a>
                        @endif
                    </h5>
